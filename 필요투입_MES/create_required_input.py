...
"opseq AS(SELECT NM_OPLINE,ROW_NUMBER() OVER(ORDER BY NM_OPLINE) AS seq FROM(SELECT DISTINCT NM_OPLINE FROM __DB__.dbo.MMIT_OPLINE WHERE CD_CORP='01' AND ISNULL(NM_OPLINE,'')<>'') x),"
...
"SELECT o.NM_OPLINE AS line,oq.seq AS seq,n.dt AS dt,i.CD_ITEM AS sub_part_no,i.NM_ITEM AS part_name "
"FROM need n JOIN op o ON o.CD_OPLINE=n.CD_OPLINE JOIN opseq oq ON oq.NM_OPLINE=o.NM_OPLINE JOIN __DB__.dbo.BSIT_ITEM i ON i.CD_SYSITEM=n.child "
...
"const seqMap={}; for(const r of cur){ if(r.line!=null&&r.seq!=null) seqMap[r.line]=r.seq; }\n"
...
"  const sp=seqMap[line]?String(seqMap[line]).padStart(2,'0')+'. ':'';\n"
"  body=`${sp}${line}라인에 출고요청이 있습니다.${ex}`; }\n"
